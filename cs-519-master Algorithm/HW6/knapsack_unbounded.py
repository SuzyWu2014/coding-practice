def best(weight, items):
    ...
    **All numbers are positive integers.**
    ...
    """
    d = [0 for i in xrange(weight + 1)]
    back = [0 for i in xrange(weight + 1)]

    for w in xrange(1, weight + 1):
        vals = [(d[w - item[0]] + item[1], i) for i, item in enumerate(items) if w >= item[0]]
        d[w] = max(vals)[0] if len(vals) > 0 else 0
        for val, i in vals:
            if val == d[w]:
                back[w] = i
                break
    count = back_trace(weight, items, back)
    return (d[weight], count)

def back_trace(weight, items, back):
