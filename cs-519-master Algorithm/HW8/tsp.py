...
def tsp(cities):
    ...
    cache = collections.defaultdict(lambda: route(distance=float("inf"),
                                                  pre_city=-1))
    # From city 0 to city i
    for i, sub in enumerate(city_sets(size, 1)):
        cache[(i + 1, sub)] = route(distance=distance(cities, 0, i + 1),
                                    pre_city=0)
    ...
    for cnt in xrange(2, size):
        for subset in city_sets(size, cnt):
            for city in subset:
                ...
                tmp = min([
                    route(distance=cache[(m, pre_set)].distance + distance(cities, m, city),
                          pre_city=m)
                    for m in pre_set])
                cache[(city, subset)] = min(cache[(city, subset)], tmp)

    # Back to city 0
    cache[(0, tuple(xrange(0, size)))] = min([
        route(distance=cache[(i, tuple(xrange(1, size)))].distance + distance(cities, i, 0),
              pre_city=i)
